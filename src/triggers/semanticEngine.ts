...
function summaryConditions(adventure: Adventure): SemanticCondition[] {
  ...
  const messagesSinceLast = adventure.messages.length - lastIndex;
  if (messagesSinceLast < everyN) return [];
  return [{
    id: "summary:rolling",
    label: "Durable Summary",
    condition: `when a permanent story development has occurred — completed arc beat, major consequence, relationship change, or revealed truth — that is NOT already captured in the current durable summary`,
    sourceType: "summary" as const,
    actionFactory: () => [{ type: "updateSummary" as const }],
  }];
}
...
function makeProposal(
  ...
  return {
    ...
    appendContent: fields.proposedType === "plotEssentialsUpdate" || fields.proposedType === "summaryUpdate",
    createdAt: now,
    updatedAt: now,
  };
}
...
async function generatedActionsFor(
  ...
  try {
    ...
    if (triggerAction.type === "updateSummary") {
      const { messages: summaryMessages } = buildRollingSummaryPayload(adventure);
      const response = await sendOpenAICompatibleChatCompletion({ config: evaluationConfig(adventure, providerConfig), messages: summaryMessages });
      if (accum && response.usage) { accum.promptTokens += response.usage.promptTokens ?? 0; accum.completionTokens += response.usage.completionTokens ?? 0; }
      const additions = response.content.replace(/<think>[\s\S]*?<\/think>/gi, "").trim();
      if (!additions) return { actions: [] };
      const proposal = makeProposal({ proposedType: "summaryUpdate", title: "Durable Summary", content: additions, rationale: "New permanent story developments." }, adventure);
      return {
        actions: [{ type: "ADD_MEMORY_PROPOSAL", proposal }],
        generated: { targetType: "component", targetId: "summary", title: "Durable Summary", preview: preview(additions) },
      };
    }
