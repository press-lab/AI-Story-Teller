...
function expectExactPayloadFromPreview(adventure: Adventure, mode: MemoryPriorityMode) {
  ...
  const lengthHint = `PLAYABILITY RESPONSE TARGET: Return one story entry sized to the player's selected length: about ${wordTarget} visible narrative words, with a normal range of ${Math.floor(wordTarget * 0.8)}-${maxWords} words. Hidden <thought> and <memory> tags do not count toward that visible word target. Stay inside that range unless obeying it would visibly cut off the active beat. Continue the active scene as a substantial playable segment: carry through immediate consequences, NPC reactions, dialogue, and motion that naturally follow from the player's input. Do not summarize, fade out, wrap up, or end the scene just because the response target is reached. Stop only before deciding a meaningful player choice, resolving a major outcome the player has not earned, touring multiple unrelated locations, or introducing a full cast. End on an in-scene moment that invites the next action without announcing choices.`;
