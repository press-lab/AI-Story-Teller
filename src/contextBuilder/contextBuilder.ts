...
export function buildContext(adventure: Adventure, options: BuildOptions = {}): ContextBuildResult {
  ...
  const lengthHintText = `PLAYABILITY LENGTH REQUIREMENT: Compose one story entry that satisfies the player's selected length setting. For this turn, the entry should feel like a ${wordTarget}-word response, with an acceptable visible range of ${Math.floor(wordTarget * 0.8)}-${maxWords} words. Hidden <thought> and <memory> tags do not count toward this requirement. Treat the range as a pacing requirement, not a command to mention word counts or pad prose. If the range conflicts with a clean playable beat, finish the beat cleanly instead of stopping mid-action. Continue the active scene as a substantial playable segment: carry through immediate consequences, NPC reactions, dialogue, and motion that naturally follow from the player's input. Do not summarize, fade out, wrap up, or end the scene just because the target length is near. Stop only before deciding a meaningful player choice, resolving a major outcome the player has not earned, touring multiple unrelated locations, or introducing a full cast. End on an in-scene moment that invites the next action without announcing choices.`;
